"""deck_mro - U.S. Navy & USCG Vessel MRO market-sizing PowerPoint build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/mro/deck/``) so ``deck_mro``
    resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/mro/deck/   (holds deck_mro + build_deck.py)
