"""workbook_mro - Navy & USCG Vessel MRO spend / TAM workbook build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/mro/workbook/``) so
    ``workbook_mro`` resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/mro/workbook/   (holds workbook_mro + build_workbook.py)
