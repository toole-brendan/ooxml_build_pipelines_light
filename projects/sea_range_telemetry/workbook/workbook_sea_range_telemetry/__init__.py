"""workbook_sea_range_telemetry - Sea Range Telemetry workbook build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/sea_range_telemetry/workbook/``) so
    ``workbook_sea_range_telemetry`` resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/sea_range_telemetry/workbook/  (holds workbook_sea_range_telemetry + build_workbook.py)
