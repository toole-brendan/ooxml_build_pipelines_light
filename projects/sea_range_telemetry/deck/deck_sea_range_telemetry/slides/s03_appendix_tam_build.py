...
spend) - drop into a central collector that flows into the annual TAM output rail on the
right (light body, dark cap; U.S. TAM, Europe TAM, Total TAM). Arithmetic operators
appear only inside formulas. Two
...
_TOPIC            = "TAM Build"
_TAKEAWAY = ("Source evidence is normalized into an annual TAM spend pool before "
             "ASV role-suitability filters apply.")
...
def _engine_card(base, x, y, fill, fg, cap, lines, body_align, result, result_size,
                 chips) -> str:
    """One engine card: solid fill (sharp rectangle), a cap + formula/component stack
    + bold result, and an optional row of bottom chips (fill=None -> a no-fill italic
    caption)."""
...
def _rail() -> str:
    """Annual-TAM output rail (light body, dark cap): a dark cap labels the pool, a
    light body carries three formula bands + a bottom qualifier in black text,
    separated by thin black dividers. The dark fill is reserved for the cap so the
    rail reads as the final output without a full dark body."""
    ...
    cap = text_box(
        ...
        insets=(120_000, 20_000, 120_000, 20_000))

    def _band(sp_id, name, by, bh, label, formula):
        return text_box(
            sp_id, name, _RAIL_X, by, _RAIL_W, bh,
            [paragraph([run(label, size=EXHIBIT_HEADER_13PT, bold=True, color=BLACK,
                            font=FONT)], space_after=120, line_spacing=104_000),
             paragraph([run(formula, size=LABEL_9PT, color=BLACK, font=FONT)],
...
def _body() -> str:
    ...
    europe = _engine_card(
        50, _COL2_X, _ROW_B_Y, BLUE_1, BLACK, "EUROPE ANCHORS",
