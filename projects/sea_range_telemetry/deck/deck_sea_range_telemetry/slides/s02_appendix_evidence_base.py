...
_TAKEAWAY = ("Four source streams feed distinct model functions while assumptions "
             "remain source-mapped, bounded, and checkable.")
...
def _fields_card(sp_id, i) -> str:
    # Quieter structural column: no fill/border, just a thin lane-colored accent band
    # at the left edge (the source-family color ramp) and text inset clear of it.
    accent = _SOURCES_CARDS[i][2]
    band = text_box(sp_id + 50, "FieldsAccent", _C2_X, _lane_y(i) + 45_000, 55_000,
                    _LANE_H - 90_000, [paragraph([])], fill=accent,
                    line_color=None, anchor="t", insets=INSETS_NONE)
    card = text_box(sp_id, "FieldsCard", _C2_X, _lane_y(i), _C2_W, _LANE_H,
                    _fields_paras(i), fill=None, line_color=None, anchor="ctr",
                    insets=(150_000, 30_000, 70_000, 30_000))
    return band + card
...
def _chip_stack(base_id, i) -> str:
    ...
    for j, txt in enumerate(chips):
        ...
        parts.append(text_box(
            ...
            fill=fill, prst="roundRect", geom_adj={"adj": 12_000}, anchor="ctr",
            insets=(40_000, 14_000, 40_000, 14_000)))
    return "".join(parts)


def _example_card(sp_id, i) -> str:
    # Unboxed: drop the black border, define the column with a thin left rule and
    # nudge the text clear of it.
    lines = _EXAMPLES[i]
    paras = [paragraph([run(t, size=_SZ_82, color=BLACK, font=FONT)],
                       line_spacing=100_000) for t in lines]
    rule = connector(sp_id + 10, "ExampleRule", _C4_X, _lane_y(i) + 45_000, 0,
                     _LANE_H - 90_000, color=BLACK, width=9_525)
    card = text_box(sp_id, "ExampleCard", _C4_X, _lane_y(i), _C4_W, _LANE_H, paras,
                    fill=None, line_color=None, anchor="ctr",
                    insets=(170_000, 24_000, 60_000, 24_000))
    return rule + card
