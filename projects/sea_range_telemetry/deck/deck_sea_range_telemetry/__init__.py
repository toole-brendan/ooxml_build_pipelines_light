"""deck_sea_range_telemetry - Sea Range Telemetry PowerPoint build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/sea_range_telemetry/deck/``) so
    ``deck_sea_range_telemetry`` resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/sea_range_telemetry/deck/   (holds deck_sea_range_telemetry + build_deck.py)
