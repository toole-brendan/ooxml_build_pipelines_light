"""Sea Range Telemetry deck pipeline bindings.
...
deck_sea_range_telemetry/__init__.py.
"""
...
DECK_DIR = Path(__file__).resolve().parents[1]      # projects/sea_range_telemetry/deck/
...
OUT = PROJECT_DIR / "20260607_Sea Range Telemetry_vS.pptx"
...
_CREATOR = "deck_sea_range_telemetry build_deck.py"
_APP = "deck_sea_range_telemetry"


def build() -> int:
    """Render every registered slide and package into the output .pptx."""
    from deck_sea_range_telemetry.slides import SLIDE_RENDERS
    if not SLIDE_RENDERS:
        raise SystemExit(
            "deck_sea_range_telemetry/slides/__init__.py SLIDE_RENDERS is empty - add a "
            "slide module before building. Start by copying "
            "deck_core/slide_base_template.py to deck_sea_range_telemetry/slides/cover.py, "
            "then register it in deck_sea_range_telemetry/slides/__init__.py."
        )
