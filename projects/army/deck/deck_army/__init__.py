"""deck_army - U.S. Army Market Mapping PowerPoint build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/army/deck/``) so ``deck_army``
    resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/army/deck/   (holds deck_army + build_deck.py)
