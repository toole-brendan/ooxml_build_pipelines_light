...
S_NOTE's gray italic). The intro captions read as italicized lines sitting directly
under each title banner, so we register an italic-black style ONCE by appending to
workbook_core.styles.CELL_XFS at build time, IN THIS PROCESS ONLY - the same scoping
trick _text_input.py uses. build_styles_xml() reads CELL_XFS at build time, so
