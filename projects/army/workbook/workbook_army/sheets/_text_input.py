...
house rule is 'color = numeric cells only'. A raw data sheet may carry one genuine
hardcoded text identity (e.g. a PIID / UEI key) that should read as source input,
not derived text. So we register a blue text style ONCE by appending to
workbook_core.styles.CELL_XFS at build time, IN THIS PROCESS ONLY (the same scoping
trick lib.py uses for tab colors). build_styles_xml() reads CELL_XFS at build time,
so appending here - at sheet import, before packaging - is enough; no workbook_core
source change, and every other pipeline builds in its own process with unchanged
style tables.
