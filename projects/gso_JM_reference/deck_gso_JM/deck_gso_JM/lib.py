"""GS&O style-guide mini-deck pipeline bindings.
...
DECK_DIR = Path(__file__).resolve().parents[1]      # projects/gso_JM_reference/deck_gso_JM/
PROJECT_DIR = Path(__file__).resolve().parents[2]   # projects/gso_JM_reference/  (build output lands here)
ROOT = Path(__file__).resolve().parents[4]          # workspace root (holds deck_core/ + infra/)

OUT = PROJECT_DIR / "20260615_GSO_Strategy Materials Style Guide_vS.pptx"
...
IMAGES = DECK_DIR / "deck_gso_JM" / "slides" / "images"  # per-deck pictures (logos etc.); created on demand

_TITLE = "GS&O Strategy Materials Style Guide (faithful port)"
_CREATOR = "deck_gso_JM build_deck.py"
_APP = "deck_gso_JM"


def build() -> int:
    """Render every registered slide and package into the output .pptx."""
    from deck_gso_JM.slides import SLIDE_RENDERS
    if not SLIDE_RENDERS:
        raise SystemExit(
            "deck_gso_JM/slides/__init__.py SLIDE_RENDERS is empty - add a slide "
