"""deck_awards_methodology - Awards Methodology PowerPoint build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/awards_methodology/``) so
    ``deck_awards_methodology`` resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/awards_methodology/   (holds deck_awards_methodology + build_deck.py)
_CORE_DIR = str(_HERE.parents[3])       # workspace root                       (holds deck_core)
