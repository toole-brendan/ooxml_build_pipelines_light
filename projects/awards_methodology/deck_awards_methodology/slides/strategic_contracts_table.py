...
from deck_core.style import IN, PT, BLACK, WHITE, GRAY_3, FONT
...
_GREEN, _RED, _GREY = "1B8A57", "A33124", "808080"
_BFILL  = {"F": _GREEN, "U": GRAY_3, "X": _RED}      # swatch fill
_BLABEL = {"F": "Favorable", "U": "Unknown", "X": "Unfavorable"}
_BTEXT  = {"F": WHITE, "U": BLACK, "X": WHITE}       # label colour on the swatch
...
_HEAVY = {"color": BLACK, "width": 12700}      # 1pt black (header / group boundary)
...
def _hcell(text):
    return tcell(text, size=PT(11), bold=True, color=BLACK, align="ctr", anchor="ctr",
                 borders={"T": "none", "B": _HEAVY, "L": "none", "R": "none"})


def _route_cell(line1, line2, span, bottom):
    return tcell_rich([
        tpara([trun(line1, size=PT(12), bold=True, color=WHITE, font=FONT)], align="ctr", mar_l=0, indent=0),
        tpara([trun(line2, size=PT(8), color=WHITE, font=FONT)], align="ctr", mar_l=0, indent=0),
    ], row_span=span, anchor="ctr", fill=_ROUTE_FILL,
       borders={"T": "none", "B": bottom, "L": "none", "R": {"color": WHITE, "width": 6350}})
...
def _drow(rec, route_cell, bmode):
    ...
    cells.append(tcell_rich([
        tpara([trun(plat, size=PT(11), bold=True, color=BLACK, font=FONT)], mar_l=0, indent=0),
        tpara([trun(piid, size=PT(8), italic=True, color=_GREY, font=FONT)], mar_l=0, indent=0),
    ], anchor="ctr", borders=edge))
    cells.append(tcell(inc, size=PT(10), color=BLACK, align="ctr", anchor="ctr", borders=edge))
    cells.append(tcell(amt, size=PT(10), bold=True, color=BLACK, align="ctr", anchor="ctr", borders=edge))
    cells.append(tcell(sig, size=PT(9), color=BLACK, anchor="ctr", borders=edge))
    cells.append(tcell(fund, size=PT(9), color=BLACK, align="ctr", anchor="ctr", borders=edge))
