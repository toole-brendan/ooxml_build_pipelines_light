...
EXHIBIT — "Strategic Contracts": the worked output of the
deck's method. An eight-column recompete watch list of real defense-maritime contracts grouped by the slide-2
...
from deck_core.primitives import (
    slide, table, trow, tcell, tcell_rich, tpara, trun,
...
# table height is deterministic. The route spine is deliberately wider than the raw draft
# and uses manual title-line breaks to prevent PowerPoint from splitting a word.
# Verified via deck_core.text_metrics at a conservative width. Σ widths = 12.30.
_COLW = [IN(1.15), IN(2.60), IN(1.85), IN(1.09), IN(1.98), IN(1.36), IN(1.05), IN(1.22)]
...
def _route_cell(title_lines, sub_lines, span, bottom):
    paras = []
    for i, line in enumerate(title_lines):
        paras.append(tpara([trun(line, size=PT(11), bold=True, color="FFFFFF", font=FONT)],
                           align="ctr", mar_l=0, indent=0,
                           space_after=300 if i == len(title_lines) - 1 else 0))
    for line in sub_lines:
        paras.append(tpara([trun(line, size=PT(8), color="FFFFFF", font=FONT)],
                           align="ctr", mar_l=0, indent=0))
    return tcell_rich(paras, row_span=span, anchor="ctr", fill=_ROUTE_FILL,
                      l_ins=22860, r_ins=22860, t_ins=45720, b_ins=45720,
                      borders={"T": "none", "B": bottom, "L": "none",
                               "R": {"color": "FFFFFF", "width": 6350}})
...
def _body() -> str:
    ...
    # ── chrome: house breadcrumb + title/subtitle + prelim chip ──
    out.append(breadcrumb("Defense Market Strategy", "Strategic Contracts"))
    out.append(title_placeholder("Strategic Contracts",
                                 "Recompete watch list by entry route, timing, incumbents, and funding."))
    ...
    for i, rec in enumerate(_ROUTE_B):
        rc = _route_cell(("Through the", "prime"), ("Enter a locked build", "via supply chain"), n_b, "none") if i == 0 else None
    ...
    out.append(table(n(), "Table 44", IN(0.495), IN(1.60), IN(12.30), IN(5.27), col_widths=_COLW, rows=rows))
    return "".join(out)
