"""Schematics archetype reference-port pipeline bindings.
...
the shared builder ``deck_core.lib.build_pptx``.
...
DECK_DIR = Path(__file__).resolve().parents[1]      # .../archetypes/schematics_curated/
ROOT = Path(__file__).resolve().parents[5]          # workspace root (holds deck_core/ + infra/)

OUT = DECK_DIR / "Schematics (curated reference).pptx"
...
IMAGES = DECK_DIR / "schematics" / "slides" / "images"  # per-deck pictures (converter-populated)

_TITLE = "Schematics (curated reference)"
_CREATOR = "schematics_curated build_deck.py"
_APP = "schematics_curated"


def build() -> int:
    """Render every registered slide and package into the output .pptx."""
    from schematics.slides import SLIDE_RENDERS
    if not SLIDE_RENDERS:
        raise SystemExit(
            "schematics/slides/__init__.py SLIDE_RENDERS is empty - "
