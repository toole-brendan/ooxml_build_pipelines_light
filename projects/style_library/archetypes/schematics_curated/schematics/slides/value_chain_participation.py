...
CODE MAP (body follows source PAINT ORDER; headers mark roles in place):
  ...
  • _STEP_CHEVRONS ... the value-chain step header chevrons
  • _PLAYER_BOXES .... player-archetype boxes down the matrix
  • _GROUP_LABELS .... small italic group annotations
...
Converter stats: text_box=7, connector=5, table=2, picture=24, chrome_builders=3,
clusters=4 (covering 14 shapes), dropped=2 (think-cell OLE frame + 1 empty <p:pic>).
"""
...
_PLAYER_BOXES = [    # (x, y, cx, cy, line, label) x5 — player-archetype boxes
...
_GROUP_LABELS = [    # (x, cx, label) x3 — small italic group annotations
...
def _body() -> str:
    ...
    out.append(connector(n(), "Straight Arrow Connector 1032", IN(4.291), _DIV_Y, IN(0), _DIV_H, color="808080", width=3175, dashed=True, flip_h=True))
    out.append(connector(n(), "Straight Arrow Connector 1036", IN(6.404), _DIV_Y, IN(0), _DIV_H, color="808080", width=3175, dashed=True, flip_h=True))
    out.append(connector(n(), "Straight Arrow Connector 1038", IN(8.526), _DIV_Y, IN(0), _DIV_H, color="808080", width=3175, dashed=True, flip_h=True))
    out.append(connector(n(), "Straight Arrow Connector 1040", IN(10.631), _DIV_Y, IN(0), _DIV_H, color="808080", width=3175, dashed=True, flip_h=True))
    ...
    # ── player-archetype boxes ──
    for _x, _y, _cx, _cy, _lc, _t in _PLAYER_BOXES:
    ...
    out.append(text_box(n(), "Rectangle 20", IN(0.495), IN(1.361), _MARK_W, _MARK_W, [paragraph([], align="ctr")], fill=None, line_color="0E1924", line_width=19050, anchor="ctr"))
    # ── group labels ──
    for _x, _cx, _t in _GROUP_LABELS:
        out.append(text_box(n(), "Label", IN(_x), _GROUP_Y, IN(_cx), _GROUP_H, [paragraph([run(_t, size=PT(8), color=BLACK, font=FONT)])], fill=None, line_color="none", anchor="ctr", wrap="none"))
    out.append(text_box(n(), "Rectangle 24", IN(4.457), IN(1.361), _MARK_W, _MARK_W, [paragraph([], align="ctr")], fill=WHITE, line_color=BLACK, line_width=19050, dashed_line=True, anchor="ctr"))
    ...
    out.append(text_box(n(), "Rectangle 1058", IN(2.691), IN(1.361), _MARK_W, _MARK_W, [paragraph([], align="ctr")], fill=None, line_color="447BB2", line_width=19050, anchor="ctr"))
    # ── more logos ──
