"""Distributed Shipbuilding Consolidated deck pipeline bindings.
...
DECK_DIR = Path(__file__).resolve().parents[1]      # projects/distributed_shipbuilding/deck/
...
OUT = PROJECT_DIR / "20260605_Defense Demand Drivers New Construction_v1.0.pptx"
...
_CREATOR = "deck_consolidated build_deck.py"
_APP = "deck_consolidated"


def build() -> int:
    """Render every registered slide and package into the output .pptx."""
    from deck_consolidated.slides import SLIDE_RENDERS
    if not SLIDE_RENDERS:
        raise SystemExit(
            "deck_consolidated/slides/__init__.py SLIDE_RENDERS is empty - add a "
            "slide module before building. Start by copying "
            "deck_core/slide_base_template.py to deck_consolidated/slides/cover.py, "
            "then register it in deck_consolidated/slides/__init__.py."
        )
