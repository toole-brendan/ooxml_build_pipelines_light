"""deck_primary - New Construction methodology deck build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/distributed_shipbuilding/deck_primary/``)
    so ``deck_primary`` resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/distributed_shipbuilding/deck_primary/
