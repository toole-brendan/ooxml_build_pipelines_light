"""deck_data_reference - single-slide Data Reference deck build pipeline.

Thin per-deck package: binds the output path, the shared template + brand
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/distributed_shipbuilding/deck_data_reference/``)
    so ``deck_data_reference`` resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/distributed_shipbuilding/deck_data_reference/
