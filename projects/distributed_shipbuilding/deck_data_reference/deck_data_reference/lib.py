"""Data Reference deck pipeline bindings.
...
deck_data_reference/__init__.py.
"""
...
DECK_DIR = Path(__file__).resolve().parents[1]      # projects/distributed_shipbuilding/deck_data_reference/
PROJECT_DIR = Path(__file__).resolve().parents[1]   # build output lands here
ROOT = Path(__file__).resolve().parents[4]          # workspace root (holds deck_core/ + infra/)

OUT = PROJECT_DIR / "20260626_Distributed Shipbuilding Data Reference_vS.pptx"

# Shared build chrome lives once under infra/ (not vendored per deck).
...
_TITLE = "Distributed Shipbuilding - Data Reference"
_CREATOR = "deck_data_reference build_deck.py"
_APP = "deck_data_reference"


def build() -> int:
    """Render every registered slide and package into the output .pptx."""
    from deck_data_reference.slides import SLIDE_RENDERS
    if not SLIDE_RENDERS:
        raise SystemExit(
            "deck_data_reference/slides/__init__.py SLIDE_RENDERS is empty - add a "
            "slide module before building."
        )
