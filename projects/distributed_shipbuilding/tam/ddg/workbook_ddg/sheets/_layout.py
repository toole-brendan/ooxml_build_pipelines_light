"""_layout - a local row cursor for the DDG sheet modules.

Adapted (copy-from, per the workbook_core "snippets stay copy-from" principle) from
the RowCursor snippet in workbook_core/sheet_snippets.md, and kept byte-identical to
the submarines workbook's workbook_submarines/sheets/_layout.py so the two sibling
pipelines stay structurally the same. It lives inside workbook_ddg so the shared engine stays
untouched; it just composes the workbook_core primitives (banner_row / write_row /
total_row).
...
with_gutter=True, matching every DDG sheet.
"""
