"""workbook_submarines - Distributed Shipbuilding Submarines workbook build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/distributed_shipbuilding/submarines/workbook/``) so
    ``workbook_submarines`` resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/distributed_shipbuilding/submarines/workbook/  (holds workbook_submarines + build_workbook.py)
