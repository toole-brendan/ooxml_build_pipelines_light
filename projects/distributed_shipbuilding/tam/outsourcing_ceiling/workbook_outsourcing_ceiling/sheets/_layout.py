"""_layout - a local row cursor for the outsourcing-ceiling sheet modules.

Copy-from (per the workbook_core "snippets stay copy-from" principle) of the
RowCursor used across the sibling distributed-shipbuilding workbooks. It lives
inside this pipeline so the shared engine stays untouched; it just composes the
...
producer captures load-bearing positions straight from the writing call:
    ...
    c = RowCursor(4)
    pos["core"] = c.write(["Core %", f"={x}"], styles=[S_DEFAULT, S_PCT])
    ...
    def core_pct_cell(): return f"'{TAB}'!C{pos['core']}"

This keeps the safety invariant "the accessor row derives from the same value
used to write the row" structural - there is no second literal to drift.

Self-referential formulas (a cell whose formula names its own row, e.g.
`=C{r}*(1-D{r})`) are written by passing a CALLABLE in the values list; the
cursor calls it with the row it just assigned:

    c.write([name, f"={obs}", 0, lambda r: f"=C{r}+D{r}"], styles=[...])

Content lives in columns B+ (gutter mode); start_col defaults to 1 and banners
use with_gutter=True, matching every sibling sheet.
"""
