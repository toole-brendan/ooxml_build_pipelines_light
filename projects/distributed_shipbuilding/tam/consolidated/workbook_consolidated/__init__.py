"""workbook_consolidated - Distributed Shipbuilding Consolidated workbook build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/distributed_shipbuilding/workbook/``) so
    ``workbook_consolidated`` resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/distributed_shipbuilding/workbook/  (holds workbook_consolidated + build_workbook.py)
