...
Promoted accessors:
  ...
  fydp_portfolio_gross_cell(fy)   constant-FY2026 submarine portfolio (Va + Col) cell
"""
...
_PROGRAMS = [(2013, "Virginia"), (1045, "Columbia"), (2122, "DDG-51")]
_SUB_LIS = (2013, 1045)
...
def _make():
    ...
    c.section("§5 - Constant FY2026 $M", _NCOLS)
    c.blank()
    ...
    g13, g10 = const[2013], const[1045]
    port = c.total(["Submarine portfolio (Va + Col), constant FY2026 $M"]
    ...
    def fydp_portfolio_gross_cell(fy):
        if fy not in _FY_COL:
            raise ValueError(f"FY {fy!r} outside {_FY!r}")
        return f"'{TAB_FYDP}'!{_FY_COL[fy]}{port}"
    ...
    return (SheetEntry(TAB_FYDP, _GROUP, render),
            fydp_gross_then_cell, fydp_gross_cell, fydp_qty_cell, fydp_portfolio_gross_cell)


(FYDP_OUTYEARS, fydp_gross_then_cell, fydp_gross_cell, fydp_qty_cell,
 fydp_portfolio_gross_cell) = _make()
