...
from workbook_master_tam.sheets._tabs import TAB_CHECKS
from workbook_master_tam.sheets._periods import OY as _OY
from workbook_master_tam.sheets import virginia_tam as VA
from workbook_master_tam.sheets import columbia_tam as CO
from workbook_master_tam.sheets import ddg_tam as DD
from workbook_master_tam.sheets import assumptions as A
...
def _make():
    ...
    _bounds("Virginia applied BC coefficient", VA.applied_coeff_cell(),
            lo=0, hi=1, lo_strict=True)
    _bounds("Columbia applied BC coefficient", CO.applied_coeff_cell(),
            lo=0, hi=1, lo_strict=True)
    _bounds("DDG-51 applied BC coefficient", DD.applied_coeff_cell(),
            lo=0, hi=1, lo_strict=True)
    _bounds("Virginia OBBBA modeled BC share", A.obbba_bc_share_cell(2013),
    ...
    c.write(["Check", "Min margin $M", "Status"],
            styles=[S_HEADER_LEFT, S_HEADER_CENTER, S_HEADER_CENTER])
    _band("Virginia outyear high >= low", VA)
    _band("Columbia outyear high >= low", CO)
