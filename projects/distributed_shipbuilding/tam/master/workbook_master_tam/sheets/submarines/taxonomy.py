"""taxonomy - shared work-type bucket vocabulary + the subaward classifier.

This is a NON-sheet helper module (it renders nothing and is NOT registered in
SHEETS). It holds the one load-bearing cross-sheet vocabulary of the workbook -
the seven work-type buckets, the NAICS-4 -> bucket crosswalk, the name-key lists,
and ``classify()`` - so that every sheet module that needs them (methodology_scope
renders them; entity_master classifies vendors; assumptions_controls / sam_build /
worktype_evidence iterate the bucket list; the register/validation tabs need
BUCKET_KEYS) imports from ONE place instead of from a rendering module.

Keeping the vocabulary here (rather than inside methodology_scope) means a sheet
module that consumes the taxonomy never has to import the methodology *renderer*,
which keeps the import graph clean: taxonomy has no sheet dependencies, so it is a
true leaf every other module can depend on.
"""
