...
from workbook_master_tam.sheets.submarines.data_obbba_funding import (
...
from workbook_master_tam.sheets.submarines.data_entity_master import addressable_total_cell
...
from workbook_master_tam.sheets.submarines.inputs_assumptions import scenario_name
from workbook_master_tam.sheets.submarines._layout import RowCursor

_GROUP = "outputs"
_TAB = "Sub Figure Register"
...
# Master-workbook namespacing: defined names are workbook-unique, so each
# program's figure register registers its set under a program prefix.
_DN_PREFIX = "sub_"
_DEFINED = [
...
def _render_figure_register() -> WorksheetSpec:
    ...
    for nm in _DEFINED:
        c.write([_DN_PREFIX + nm, dn_target[nm], _DN_MEANING[nm]], styles=[S_DEFAULT, S_DEFAULT, S_DEFAULT], outline_level=1)
    c.blank(2)
    ...
    defined_names = {_DN_PREFIX + nm: _abs(dn_target[nm]) for nm in _DEFINED}
