...
from workbook_master_tam.sheets._tabs import TAB_SCN_BUDGET
...
# title(2)+italic(3)+2 blanks+§1 banner(6)+blank+header(8)+3 program rows(9-11)+
# portfolio(12)+local deflator factor row(13)+2 blanks -> detail at 16.
_FACTOR_ROW = 13     # one local "Constant-FY2026 factor" row; the §Nb const blocks
                     # multiply by THIS row (6 cells) instead of 186 cross-sheet refs.
_DETAIL_BASE = 16
...
_PROGRAMS = [(2013, "Virginia", _SUB_METRICS),
             (1045, "Columbia", _SUB_METRICS),
             (2122, "DDG-51", _DDG_METRICS)]
