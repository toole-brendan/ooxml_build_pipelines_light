...
Each program's applied BC supplier coefficient = dollar-weighted (other-US + foreign)
over its own BC-stream rows (master + disclosed): Virginia ~34%, Columbia ~22%, DDG-51
~25.3% (MYP-corrected), DDG FY2022 ~22% (FY18-22 vintage masters).

Promoted accessors (consumed by the program TAM tabs):
  va_bc_coeff_cell, col_bc_coeff_cell, ddg_bc_coeff_cell, ddg_bc_coeff_fy22_cell
  ddg_disclosed_coeff_ref_cell, sub_ap_coeff_ref_cell   (reference)
"""
...
# title(2)+italic(3)+2 blanks+§1 banner(6)+blank+headline(8..20)+2 blanks -> body at 23
_DATA_BASE = 23
...
def _build_headline(base: int):
    ...
    P["va"] = c.write(["Virginia BC"]
                      + _coeff_cells(f"{_prog('Virginia')}*{_BC}"),
                      styles=[S_BOLD, S_NUM, S_NUM, S_PCT])
    P["col"] = c.write(["Columbia BC"]
                       + _coeff_cells(f"{_prog('Columbia')}*{_BC}"),
                       styles=[S_BOLD, S_NUM, S_NUM, S_PCT])
    ...
    P["sub_ap"] = c.write(["Submarine AP/LLTM"]
                          + _coeff_cells(f"({_prog('Virginia')}+{_prog('Columbia')})*{_AP}"),
                          styles=[S_DEFAULT, S_NUM, S_NUM, S_PCT])
...
def va_bc_coeff_cell() -> str: return f"'{TAB_POP}'!E{_HP['va']}"
def col_bc_coeff_cell() -> str: return f"'{TAB_POP}'!E{_HP['col']}"
...
def sub_ap_coeff_ref_cell() -> str: return f"'{TAB_POP}'!E{_HP['sub_ap']}"
