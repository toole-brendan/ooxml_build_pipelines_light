...
Strategy:
  - read extracted/nc_lifetime_vendors.csv (already sorted by lifetime $ desc)
  ...
  - fallback: if samRegistered=Yes empty, retry without that filter
...
TOP_N = 150  # ~90% of lifetime dollars; well under 1000/day quota
...
# 2-digit NAICS sector → English label, for human-readable grouping
NAICS_SECTORS = {
...
def call_entity(api_key, uei, sam_registered="Yes"):
    ...
    slowness against api.sam.gov (lookups take 1-2 min). curl behaves like
    the smoke tests confirmed (~0.3s/call), so we shell out.
    """
...
def extract_naics(entity_data):
    """Pull primary NAICS code + description from an entityData[0] record."""
    if not entity_data:
...
def lookup_uei(api_key, uei, vendor_name):
    """Cached + fallback Entity lookup. Returns dict suitable for CSV row.

    Note: the samRegistered=No retry is intentionally OMITTED. Empirically,
    the No-branch is extremely slow (server appears to do a full-dataset scan
    when no filter narrows the result) while the Yes-branch returns in ~0.3s.
    Currently-registered SAM entities are sufficient for our top-N enrichment;
    UEIs that miss Yes are recorded as not_found.
    """
    ...
    if cache_path.exists():
        ...
        time.sleep(0.2)  # polite delay (API itself is ~0.3s)
...
def main():
    ...
    # Read top-N vendors from the new-construction lifetime list
    ...
    # Quick summary by 4-digit NAICS
