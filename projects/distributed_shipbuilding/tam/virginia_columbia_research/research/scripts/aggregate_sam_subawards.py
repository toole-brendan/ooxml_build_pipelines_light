...
Outputs (in extracted/):
  ...
  sam_subaward_full_dedup_check.txt   Diagnostics: total records, unique
                                      subAwardReportId, suspected duplicates
"""
import csv
import json
import os
...
def fy_from_date(d):
    """US federal FY: Oct 1 → Sep 30. e.g. 2024-10-15 → FY25."""
    if not d:
        return None
    try:
...
def load_all_sam_records():
    """Return list of (piid, record_dict) tuples from all SAM JSON files,
...
def build_sub_to_parent_lookup(records):
    """Two-pass: collect subEntityUei → subParentUei from records that have both.
    Then we can back-map records that lack parent UEI."""
    lookup = {}
    for _, r in records:
...
def recipient_key_and_name(r, sub_to_parent_lookup):
    """Return (canonical_key, human_name) for a record.
    Prefer subParentUei → back-mapped parent UEI → subEntityUei → name fallback.
    Name prefers subEntityParentLegalBusinessName → subEntityLegalBusinessName."""
...
def dedup_check(records):
    """Verify SAM data dedup-clean per howto. Reports collisions."""
    ids = defaultdict(int)
    blanks = 0
    for _, r in records:
...
def aggregate_top_parents(records, sub_to_parent_lookup, top_n=200):
    ...
    for _, r in records:
        ...
        slot["count"] += 1
        # Prefer most-common name — naive: take the longest non-blank
...
def write_sam_vs_usaspending_csv():
    """Side-by-side: for each seed PIID, record counts + $ totals from each source.
    This quantifies the recovered long-tail."""
    # Load SAM summary
...
def main():
    ...
    # Top 15 quick stdout
    print("\nTop 15 parent recipients (across all submarine PIIDs):")
