"""doc_distributed_shipbuilding - Distributed Shipbuilding Word build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/distributed_shipbuilding/doc/``)
    so ``doc_distributed_shipbuilding`` resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/distributed_shipbuilding/doc/   (holds doc_distributed_shipbuilding + build_doc.py)
_CORE_DIR = str(_HERE.parents[5])       # workspace root                          (holds docx_core)
