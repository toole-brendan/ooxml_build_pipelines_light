"""deck_ddg - Distributed Shipbuilding Destroyers PowerPoint build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/distributed_shipbuilding/ddg/deck/``) so ``deck_ddg``
    resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/distributed_shipbuilding/ddg/deck/   (holds deck_ddg + build_deck.py)
