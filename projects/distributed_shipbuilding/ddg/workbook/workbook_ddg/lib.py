"""Destroyer Outsourced Construction pipeline bindings.
...
is set up in workbook_ddg/__init__.py.
"""
...
WORKBOOK_DIR = Path(__file__).resolve().parents[1]   # projects/distributed_shipbuilding/ddg/workbook/
PROJECT_DIR = Path(__file__).resolve().parents[2]    # projects/distributed_shipbuilding/ddg/   (build output lands here)
REPO_ROOT = Path(__file__).resolve().parents[5]      # ooxml_build_pipelines_light/
OUT = PROJECT_DIR / "20260601_Distributed Shipbuilding DDG_vS.xlsx"
...
REGISTRY_CSV = REPO_ROOT / "projects/research_shared/supplier_bucketing/vendor_evidence_registry.csv"

_TITLE = "Destroyer Outsourced Construction - TAM & SAM Sizing"
_CREATOR = "workbook_ddg build_workbook.py"
_APP = "workbook_ddg"
...
def build() -> int:
    ...
    from workbook_ddg.sheets import SHEETS
