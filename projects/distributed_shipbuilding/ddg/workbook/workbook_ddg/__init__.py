"""workbook_ddg - Destroyer Outsourced Construction workbook build pipeline.
...
two dirs on sys.path:
  - the build dir (this package's parent, ``projects/distributed_shipbuilding/ddg/workbook/``) so
    ``workbook_ddg`` resolves;
...
_PROJECT_ROOT = str(_HERE.parents[1])   # projects/distributed_shipbuilding/ddg/workbook/   (holds workbook_ddg + build_workbook.py)
