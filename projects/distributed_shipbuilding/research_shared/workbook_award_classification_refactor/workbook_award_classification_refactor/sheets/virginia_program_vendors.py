"""virginia_program_vendors - the refactored Virginia-class program-vendor sheet.

One row per distinct subawardee UEI on the Virginia-class program across all corpus
...
Foreign) are SUMIFS / COUNTIFS / MINIFS / MAXIFS / IF over the Virginia Subaward
...
Built from extracted/virginia_program_vendors.csv (scripts/build_program_vendors.py).
"""
...
# Source URLs is folded into hover Notes on Role / Description (note_from below)
# and no longer rendered as a column, so its width (W_URL) is dropped.
...
# --- live roll-ups over the Virginia tx leaf ---
_UEI = virginia_tx_cols("Subawardee UEI")
_DOL = virginia_tx_cols("Subaward Amount $")
_DATE = virginia_tx_cols("Subaward Date")
_CC = virginia_tx_cols("Country Code")
...
VIRGINIA_PROGRAM_VENDORS, _ = make_flat_sheet(
    tab=TAB_VIRGINIA_PROGRAM, group="model",
    csv_name="virginia_program_vendors", table_name="VirginiaProgramVendors",
    banner="§1 - Virginia-class subaward recipients",
    intro="Entity-grain roll-up: one row per Virginia-class subawardee UEI, CY2016-2025; nominal dollars. "
