"""_layout - a local row cursor for the sheet modules.

Adapted (copy-from, per the workbook_core "snippets stay copy-from" principle)
from the RowCursor in the sibling workbooks. It lives inside this package so the
shared engine stays untouched; it just composes the workbook_core primitives
(banner_row / write_row / total_row).
...
positions straight from the writing call:
    ...
    c = RowCursor(2)
    pos["bc"] = c.write(["Per-ship BC", f"={x}"], styles=[S_DEFAULT, S_LINK_NUM])
    ...
    def per_ship_bc_cell(): return f"'{TAB}'!C{pos['bc']}"

Self-referential formulas (a cell whose formula names its own row) are written by
passing a CALLABLE in the values list; the cursor calls it with the row it just
assigned:

    c.write([name, 0, lambda r: f"=C{r}/D{r}"], styles=[...])
...
class RowCursor:
    ...
    def blank(self, n: int = 1, *, outline_level: int | None = None) -> None:
        """Advance past n blank rows (emitted as empty outlined rows inside a group
        so the contiguous run is not split)."""
