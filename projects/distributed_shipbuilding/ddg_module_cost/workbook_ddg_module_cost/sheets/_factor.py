...
format - which the deflator factors need: 1.10 / 1.02 / 0.98 collapse at one
decimal, so the displayed value would no longer be the exact multiplier. Register
...
_italic.py uses. build_styles_xml() reads both lists at build time, so appending
here (at sheet import, before packaging) is enough; no workbook_core source
change, and every other pipeline builds in its own process with unchanged tables.
