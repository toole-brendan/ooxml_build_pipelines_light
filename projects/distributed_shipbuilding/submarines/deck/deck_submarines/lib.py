"""Distributed Shipbuilding Submarines deck pipeline bindings.
...
OUT = PROJECT_DIR / "20260602_Distributed Shipbuilding Submarines_vS.pptx"
...
_CREATOR = "deck_submarines build_deck.py"
_APP = "deck_submarines"


def build() -> int:
    """Render every registered slide and package into the output .pptx."""
    from deck_submarines.slides import SLIDE_RENDERS
    if not SLIDE_RENDERS:
        raise SystemExit(
            "deck_submarines/slides/__init__.py SLIDE_RENDERS is empty - add a slide "
            "module before building. Start by copying "
            "deck_core/slide_base_template.py to deck_submarines/slides/cover.py, then "
            "register it in deck_submarines/slides/__init__.py. See deck_spec.txt."
        )
