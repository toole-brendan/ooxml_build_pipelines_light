"""bucket_tam - Rank the seven work-type buckets by average annual TAM and keep the ambiguous residual visible but separate."""
from __future__ import annotations

from deck_core.charts import column_chart, graphic_frame
from deck_core.primitives import (
    slide,
    breadcrumb,
    title_placeholder,
    prelim_chip,
    sources_line,
    run,
    paragraph,
    text_box,
)
from deck_core.style import (
    BODY_X,
    BODY_Y,
    BODY_CX,
    BODY_R,
    BODY_B,
    BLUE_1,
    BLUE_3,
    BLUE_4,
    BLUE_5,
    GRAY_1,
    GRAY_3,
    BLACK,
    FONT,
    INSETS_NONE,
    INSETS_MESSAGE,
    FINEPRINT_8_5PT,
    DENSE_BODY_10PT,
    CHART_TITLE_10PT,
    MESSAGE_11PT,
)

LAYOUT = "slideLayout4"

_SECTION = "Market Sizing"
_BREADCRUMB_TOPIC = "Bucket TAM"
_TITLE_TOPIC = "Bucket TAM"
_TAKEAWAY = "Electrical and power, structural fabrication, and piping lead the opportunity"
_SOURCES = "Sources: (1) SAM.gov FFATA/FSRS records; (2) SAM.gov Entity Management API; (3) U.S. Department of the Navy SCN Justification Books"

NOTE_H = 360_000
TITLE_H = 150_000
CHART_TITLE_GAP = 220_000

_CATEGORIES = [
    "Electrical/power",
    "Structural",
    "Piping/valves",
    "Castings",
    "Coatings",
    "Machining",
    "HVAC",
]
_VALUES = [1257.1, 624.5, 519.6, 142.8, 101.5, 98.1, 61.9]
# SRT bar palette (docs/chart_conversion_spec.md): top bucket hero navy, blue ramp
# down to a light-gray tail.
_COLORS = ["1D4D68", "486D82", "89A2B0", "AFC2CC", "D8E3EB", "BEBEBE", "DBDBDB"]

_CHART = column_chart(
    mode="ranked",
    categories=_CATEGORIES,
    series=[{"name": "Average annual TAM", "values": _VALUES, "data_point_colors": _COLORS}],
    title=None,
    show_legend=False,
    value_axis_format='"$"#,##0"M"',
    show_gridlines=True,
    major_gridline_color=GRAY_1,
    major_gridline_width=3_175,
    show_value_labels=True,
    value_label_format='"$"#,##0"M"',
    value_label_size_pt=10,
    cat_label_size_pt=9,
    gap_width=80,
    cat_header="Work-type bucket",
)
CHARTS = [_CHART]


def _chart_title(text: str, x: int, y: int, cx: int, *, sp_id: int = 10) -> str:
    return text_box(
        sp_id,
        "ChartTitle",
        x,
        y,
        cx,
        TITLE_H,
        [paragraph([run(text, size=CHART_TITLE_10PT, italic=True, color=BLACK, font=FONT)])],
        fill=None,
        line_color=None,
        anchor="t",
        insets=INSETS_NONE,
    )


def _residual_strip(y: int) -> str:
    return text_box(
        30,
        "ResidualStrip",
        BODY_X,
        y,
        BODY_CX,
        650_000,
        [
            paragraph(
                [
                    run(
                        "Unbucketed / ambiguous residual: ",
                        size=MESSAGE_11PT,
                        bold=True,
                        color=BLACK,
                        font=FONT,
                    ),
                    run(
                        "~$501M per year (~$3.01B cumulative). Tracked in TAM, excluded from broad component SAM.",
                        size=MESSAGE_11PT,
                        color=BLACK,
                        font=FONT,
                    ),
                ],
                align="ctr",
            )
        ],
        fill=GRAY_3,
        line_color=BLACK,
        anchor="ctr",
        insets=INSETS_MESSAGE,
    )


def _note() -> str:
    return text_box(
        40,
        "AverageAnnualNote",
        BODY_X,
        BODY_B - NOTE_H,
        BODY_CX,
        NOTE_H,
        [
            paragraph(
                [
                    run(
                        "Values are average annual FY2022-FY2027; cumulative shown for context. Average annual is not a run-rate.",
                        size=FINEPRINT_8_5PT,
                        italic=True,
                        color=BLACK,
                        font=FONT,
                    )
                ]
            )
        ],
        fill=None,
        line_color=None,
        anchor="t",
        insets=INSETS_NONE,
    )


def _body() -> str:
    chart_x = BODY_X
    chart_y = BODY_Y + CHART_TITLE_GAP
    chart_w = int(BODY_CX * 0.88)
    residual_y = BODY_Y + 3_280_000
    chart_h = residual_y - chart_y - 140_000
    return (
        _chart_title("Average annual TAM by work-type bucket, FY2022-FY2027", chart_x, BODY_Y, chart_w)
        + graphic_frame(sp_id=20, name="BucketTAMChart", x=chart_x, y=chart_y, cx=chart_w, cy=chart_h, rId="rId2")
        + _residual_strip(residual_y)
        + _note()
    )


def render() -> str:
    return slide(
        breadcrumb(_SECTION, _BREADCRUMB_TOPIC)
        + prelim_chip()
        + title_placeholder(_TITLE_TOPIC, _TAKEAWAY)
        + _body()
        + sources_line(_SOURCES)
    )
