...
_CHART = bar_chart(
    mode="ranked", categories=["All-gated POP anchor", "AP and LLTM reference", "Applied BC supplier coefficient"],
    series=[{"name": "Coefficient view", "values": [0.518, 0.485, 0.350], "data_point_colors": [BLUE_3, BLUE_2, BLUE_5]}],
    title=None, show_legend=False, value_axis_format="0.0%", show_gridlines=True,
    ...
    gap_width=55, cat_header="Coefficient view",
)
CHARTS = [_CHART]
...
def _body() -> str:
    out = []
    chart_w = int(BODY_CX * 0.55)
    chart_y = BODY_Y + TITLE_BAND_H + 40_000
    chart_h = 2_850_000
    out.append(_chart_title(10, "Supplier coefficient evidence, %", BODY_X, BODY_Y, chart_w))
    out.append(graphic_frame(sp_id=20, name="CoefficientChart", x=BODY_X, y=chart_y, cx=chart_w, cy=chart_h, rId="rId2"))
    table_x = BODY_X + chart_w + GAP
    table_w = BODY_R - table_x
    col_w = _widths(table_w, [2.6, 1.0])
    row_h = estimate_row_heights(_ROWS, col_w, size_pt=9.0, header_size_pt=9.0, min_row_h=274_320)
    out.append(house_table(30, "EvidenceTable", table_x, chart_y, col_w, _ROWS, row_h=row_h,
                           table_skin="rule", size=900, aligns=["l", "r"]))
    read_y = chart_y + chart_h + 120_000
    out.append(_note(40, "Read", BODY_X, read_y, chart_w, 300_000,
        "Read: Evidence supports a large distributed-production layer; the model applies a stricter coefficient.",
        size=DENSE_BODY_10PT, bold_lead="Read:", italic=False, align="ctr"))
    out.append(_note(50, "Guardrail", BODY_X, BODY_B-320_000, BODY_CX, 260_000,
        "Guardrail: Higher POP views are explanatory and sensitivity views; they are not multiplied into headline TAM.",
        size=DENSE_BODY_10PT, bold_lead="Guardrail:", italic=False, align="ctr"))
    return "".join(out)
