...
NOT part of the build pipeline. The pipeline (workbook_outsourcing_ceiling/) is
strictly stdlib-only raw OOXML; this script uses openpyxl purely to read back the
*already-built* file and sanity-check it. Never import this from a sheet module
or from lib.py.

Usage:
    python3.12 validate_workbook.py                          # structural report
    python3.12 validate_workbook.py "Assumptions!C8" ...     # dump cells

openpyxl does not evaluate formulas (only Excel/LibreOffice do), so cell dumps
show the formula string for formula cells and the literal for hardcoded cells -
exactly what we want to audit the build.
"""
...
# Built workbook lands at the project root (projects/distributed_shipbuilding/), one level up from here.
OUT = Path(__file__).resolve().parent.parent / "20260615_Distributed Shipbuilding Outsourcing Ceiling_vS.xlsx"
