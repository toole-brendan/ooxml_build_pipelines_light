"""contracts_recompete_timing (copy) - Show how vehicle structure sets the recompete clock and why an expiring clock still must clear successor and access gates before it becomes addressable.

Variant of ``contracts_recompete_timing``: the five parent-term bars and their
category labels are rendered as one native, editable stacked horizontal bar chart
(``CHARTS``/``rId2``); the trigger ticks, child-order tails, today / decision
guides, routes, and the addressability gate remain slide-shape overlays pinned to
the chart's inner plot via ``_plot_geom``/``_x_at``/``_row_center``.
"""
...
from deck_core.style import (
    ...
    DENSE_BODY_10PT, MESSAGE_11PT, VALUE_14PT,
)
...
def _body() -> str:
    ...
    parts.append(_label(
        ...
        _x_at(_VEHICLES[0]["trigger"]) - _in(0.41), row0 + _in(0.10),
    ...
    parts.append(_label(
        ...
        row0 + _in(0.10), _in(0.92), _in(0.14), "ultimate end",
        size=700, italic=True, color=GRAY_4, align="ctr",
    )); sid += 1
    ...
    parts.append(text_box(
        ...
        [paragraph([
            run("ADDRESSABLE?", size=VALUE_14PT, bold=True,
