...
def _body() -> str:
    ...
    # Per-vehicle clock-rule table in the upper-right rail, as an explicit superset
    # of the five charted lanes: the five charted vehicles first in chart order,
    # then the child-level / agreement cases that have no bar (tagged "not charted"
    # inline rather than via a full separator row, which the rail has no height for).
    table_rows = [
        ["Vehicle type", "Recompete clock", "Confidence", "Note"],

        # Charted vehicles, in chart order.
        ["Standalone definitive contract",
         "Current and ultimate completion",
         "High",
         "Current may be an option decision; ultimate is outer horizon."],

        ["Single-award IDIQ",
         "Parent last date to order",
         "High",
         "Child orders may perform past the trigger."],
        ...
        # Not charted (no bar): child-level and agreement cases, tagged inline.
        ["Task or delivery order  (not charted)",
    ...
    # Caption above the table makes the chart -> table relationship explicit.
    parts.append(_label(
        sid, "VehicleClockTableCaption", right_x, BODY_Y, right_w, _in(0.16),
        "Charted vehicles first; the two (not charted) cases are reference-only.",
        size=700, italic=True, color=GRAY_4, align="ctr",
    )); sid += 1
    table_y = BODY_Y + _in(0.16)

    # Confidence chips shade by level; the two not-charted rows (6, 7) are greyed in
    # the vehicle column to subordinate them to the five charted lanes.
    cell_fills = {
        (1, 2): BLUE_3,
        (2, 2): BLUE_3,
        (3, 2): BLUE_3,
