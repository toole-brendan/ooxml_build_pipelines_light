...
Worked slide-side recipes live in deck_core/slide_snippets.md → "charts".
...
Each factory returns a dict with three parts:

    {
        ...
        "chart_rels": str    — body of ppt/charts/_rels/chartN.xml.rels (template;
                                lib.py formats it with the global chart_num)
    }
...
Slide-side usage (build the chart, expose CHARTS, place a graphic_frame at
rId2) lives in deck_core/slide_snippets.md → "charts", with worked recipes.
At build time lib.py reads each module's `CHARTS` list, writes the chart part
...
# title text is BLACK (target_copy: default exhibit text is BLACK 000000);
...
def _label_color_on(fill_hex: str | None) -> str:
    ...
    matches slide_guide.md's "WHITE on BLUE_3/4/5, GRAY_5"; tune here if the
    palette changes."""
    if not fill_hex:
        return BLACK
...
# The {chart_num} placeholder is filled in by lib.py at build time so each
    ...
    Args:
        ...
        series: list of dicts. Each dict:
            ...
            - hide_labels (bool, optional): suppress this series' data labels.
              ...
              "chip" instead (see slide_snippets.md "charts > Recipes > F").
...
def graphic_frame(
    *,
    ...
    match the slide's chart relationship (managed by deck_core.lib at
