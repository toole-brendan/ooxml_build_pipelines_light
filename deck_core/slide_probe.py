...
# Share the text-width model with deck_core.text_metrics so the probe's
# --text-estimate and the table-row estimator (text_metrics.estimate_row_heights)
# never disagree. parents[1] of …/deck_core/slide_probe.py is the workspace root
# (which holds the deck_core package).
_CORE_ROOT = str(Path(__file__).resolve().parents[1])
if _CORE_ROOT not in sys.path:
    sys.path.insert(0, _CORE_ROOT)
from deck_core import text_metrics  # noqa: E402
...
# Locked chrome sp_ids per slide_base_template.py chrome. Body slides
# (slideLayout4) auto-number, so there is no PageNumber shape.
CHROME_SP_IDS = {2: "Breadcrumb", 3: "Title", 4: "PrelimChip",
                 9999: "Sources"}

# House body box (single source of truth = slide_base_template). Used by the
# optional --table-fit info section to report a table's estimated honest height
# against the body floor. Fall back to the template's literals if it can't import.
try:
    from deck_core import slide_base_template as _tpl
    BODY_X, BODY_Y, BODY_CX, BODY_CY = _tpl.BODY
    BODY_R, BODY_B = _tpl.BODY_R, _tpl.BODY_B
except Exception:
    BODY_X, BODY_Y, BODY_CX, BODY_CY = 453_079, 1_371_600, 11_282_362, 4_498_400
    BODY_R, BODY_B = BODY_X + BODY_CX, BODY_Y + BODY_CY
...
AVG_CHAR_WIDTH_RATIO = text_metrics.AVG_CHAR_WIDTH_RATIO   # shared model
...
def _greedy_wrap(text: str, avail_w_emu: float, avg_char_w_emu: float) -> list[str]:
    """Greedy word wrap — delegates to the shared deck_core.text_metrics model
    so the probe and the table-row estimator wrap identically."""
    return text_metrics.greedy_wrap(text, avail_w_emu, avg_char_w_emu)
...
def table_fit_info(shapes: list[ShapeProbe]) -> list[dict[str, Any]]:
    ...
    just reports, per table: authored vs content-required row heights (shared
    text_metrics model), which rows are estimated shorter than their content, the
    ...
    for s in shapes:
        ...
        try:
            required = text_metrics.estimate_row_heights(
